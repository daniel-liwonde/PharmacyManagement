function showReport(title) {
  ...
  xhttp.onreadystatechange = function() {
    if(xhttp.readyState = 4 && xhttp.status == 200)
      document.getElementById(title + "_report_div").xhttp.responseText;
  };
  xhttp.open("GET", "php/report.php?action="+ title +"&start_date=" + start_date + "&end_date=" + end_date+"&cashier_id="+ cashier_id, true);
  xhttp.send();
}

function showProductReport(title) {
  var pName = document.getElementById('p_name').value;
    var pSection = document.getElementById('p_section').value;
  var start_date = document.getElementById('start_date').value;
  var end_date = document.getElementById('end_date').value;
  //alert(start_date + " " + end_date)
  var xhttp = new XMLHttpRequest();
  ...
  xhttp.open("GET", "php/report.php?action=" + title + "&start_date=" + start_date + "&end_date=" + end_date+"&pSection="+pSection +"&pName="+pName, true);
  xhttp.send();
}

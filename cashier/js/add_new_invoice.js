...
function canExpire(id) {
  ...
  xhttp.open("GET", "php/add_new_invoice.php?action=check_expire&medicine_name=" + medicine_name, false);
  xhttp.send();
...
function isService(id) {
  ...
  xhttp.open("GET", "php/add_new_invoice.php?action=check_Qty&medicine_name=" + medicine_name, false);
  xhttp.send();
  return xhttp.responseText;
}
//end can expire
...
function checkAvailableQuantity(value, id) {
  ...
  else if((value > Number.parseInt(xhttp.responseText)) && (isService(id)==0)) {
...
function addInvoice() {
  ...
  if(!notNull(customers_name.value, "customer_name_error"))
    ...
    for(var i = 1; i < row_count; i++) {
      ...
      var medicineNameId = medicine_name.getAttribute('id').slice(14,15);
      console.log(isService(medicineNameId));
      var r=canExpire(medicineNameId);
     
      //alert("can expire? check3"+r);
       //alert("is service?"+isService(medicineNameId) +"id:"+medicineNameId);
    ...
    //{
      else if(canExpire(medicineNameId)==true){
     if(!checkExpiry(expiry_date.value, medicine_name_error.getAttribute('id')) || checkExpiry(expiry_date.value, medicine_name_error.getAttribute('id')) == -1)
        medicine_name.focus();
    }
//}
     ...
     else if(isService(medicineNameId)==0){
           if(!checkQuantity(quantity.value, quantity_error.getAttribute('id')))
        quantity.focus();
     }
    ...
    for(var i = 0; i < row_count - 1; i++) {
      //alert("inserting row" +row_count+"cont"+i);
      ...
      updateStock(medicines[i].name, medicines[i].batch_id, medicines[i].quantity,medicineNameId);
    }
...
function updateStock(name, batch_id, quantity, id) {
  var xhttp = new XMLHttpRequest();
  var service =isService(id);
  //alert("is service in update:"+service);
  xhttp.onreadystatechange = function() {
    if(xhttp.readyState = 4 && xhttp.status == 200)
      xhttp.responseText;
        //alert("Stock result : " + xhttp.responseText);
  };
  xhttp.open("GET", "php/add_new_invoice.php?action=update_stock&name=" + name +"&isservice="+ service +"&batch_id=" + batch_id + "&quantity=" + quantity, true);
  xhttp.send();
}

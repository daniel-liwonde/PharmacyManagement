<?php
  ...
  function showMedicinesStock($id) {
    ...
    if($con) {
      $seq_no = 0;
      $query = "SELECT * FROM medicines INNER JOIN medicines_stock ON medicines.NAME = medicines_stock.NAME INNER JOIN purchases ON
      medicines_stock.INVOICE_NUMBER=purchases.INVOICE_NUMBER";
...
function searchMedicineStock($text, $column) {
  ...
  if($con) {
    ...
    if($column == 'EXPIRY_DATE')
      $query = "SELECT * FROM medicines INNER JOIN medicines_stock ON medicines.NAME = medicines_stock.NAME INNER JOIN 
      ...
      INNER JOIN purchases ON medicines_stock.INVOICE_NUMBER=purchases.INVOICE_NUMBER
       WHERE medicines_stock.$column = 0";
       else if($column == 'SUPPLIER_NAME')
      $query = "SELECT * FROM medicines INNER JOIN medicines_stock ON medicines.NAME = medicines_stock.NAME 
      INNER JOIN purchases ON medicines_stock.INVOICE_NUMBER=purchases.INVOICE_NUMBER
       WHERE UPPER(purchases.$column)  LIKE '%$text%'";
    else
      $query = "SELECT * FROM medicines INNER JOIN medicines_stock ON medicines.NAME = medicines_stock.NAME
      INNER JOIN purchases ON medicines_stock.INVOICE_NUMBER=purchases.INVOICE_NUMBER
       WHERE UPPER(medicines_stock.$column) LIKE '%$text%'";

    $result = mysqli_query($con, $query) or die($column."Hello".mysqli_error($con));

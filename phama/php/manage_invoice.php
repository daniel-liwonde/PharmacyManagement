<?php
 require_once("functions.php");
  ...
  if(isset($_GET["action"]) && $_GET["action"] == "search")
    ...
    handleCollection($_POST["sell_id"]);
...
function handleCollection($id)
 {
  require "db_connection.php";
  $query =mysqli_query($con,"SELECT * FROM sales WHERE SELL_ID=$id AND TAKEN=0") Or die(mysqli_error($con));
  if(mysqli_num_rows($query)>0){
  $result=mysqli_query($con,"UPDATE sales SET TAKEN=1 WHERE SELL_ID=$id")or die(mysqli_error($con));
  echo ($result)? "collected": "error occoured";
 }
 else
 {
$result=mysqli_query($con,"UPDATE sales SET TAKEN=0 WHERE SELL_ID=$id")or die(mysqli_error($con));
echo ($result)?"updated":"error occoured";
 }
}
...
<script src="js/Collection.js"></script>
    ...
    if($con) {
      $query = "SELECT * FROM sales INNER JOIN customers ON sales.CUSTOMER_ID = customers.ID WHERE sales.INVOICE_NUMBER = '$invoice_number'";
      $result = mysqli_query($con, $query) or die("proble here".mysqli_error($con));
      $row = mysqli_fetch_array($result);
      $customer_name = $row['NAME'];
      $address = $row['GENDER'];
      $contact_number = $row['CONTACT_NUMBER'];
            ...
            <?php
              ...
              while($row = mysqli_fetch_array($result)) {
                ...
                <tr>
                  ...
                  <td><?php echo $row['QUANTITY']; ?></td>
                  <td><?php echo number_format($row['MRP']); ?></td>
                  <td><?php echo number_format($row['TOTAL']); ?></td>
                   ...
                   <td>
                    <?php  if($row['TAKEN']==1) {?>
                      <form action="" method="post">
                        <input type="hidden" value="<?php echo $sell_id ?>" name="sell_id">
                   <button type="submit" title="Collected" class="btn btn-success set"><i class="fa fa-circle-check"></i></button>
                    </form>
                  <?php
              }
              else
              ...
              <form action="" method="post">
                        <input type="hidden" value="<?php echo $sell_id ?>" name="sell_id">
               <button  title="Not collected" class="btn btn-danger"><i class="fa fa-remove"></i></button>
              </form>
              <?php
              }
              ?>
    ...
    <div class="row">
      <div class="col-md-12 col-sm-12 mb-2" > <a style="margin-left:8.6%" class="btn btn-info " href="home.php?menu=0"> <i class="fa fa-arrow-left"></i>Back</a></div>
    </div>

...
class NewMedicine {
  constructor(name, packing, supplier_name) {
    this.name = name;
...
function canExpire(id) {
  ...
  xhttp.open("GET", "php/add_new_purchase.php?action=check_expire&medicine_name=" + medicine_name, false);
  xhttp.send();
  return xhttp.responseText;
}
//end can expire
...
function isService(id) {
  ...
  xhttp.open("GET", "php/add_new_purchase.php?action=check_Qty&medicine_name=" + medicine_name, false);
  xhttp.send();
  return xhttp.responseText;
}
//end can expire
  ...
  if(!notNull(suppliers_name.value, "supplier_name_error"))
    ...
    for(var i = 1; i < row_count; i++) {
     ...
     if(!checkExpiry(expiry_date.value, medicine_name_error.getAttribute('id')) || checkExpiry(expiry_date.value, medicine_name_error.getAttribute('id')) == -1)
        ...
        newMedicine[i-1] = new NewMedicine(medicine_name.value, packing.value,  suppliers_name.value);
      }
      if(!flag)
        return false;
    }
    ...
    for(var i = 0; i < row_count - 1; i++) {
      ...
      // addNewMedicine(newMedicine[i].name, newMedicine[i].packing,  newMedicine[i].supplier_name);
      alert("adding row"+row_count);
    ...
    //SaddMedicineStock(medicine_name.value,batch_id.value, expiry_date.value, quantity.value, mrp.value, rate.value);
  }
}

function addNewMedicine(name, packing, supplier_name) {
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function() {
    if(xhttp.readyState = 4 && xhttp.status == 200)
      alert("New medicine " + xhttp.responseText);
  };
  xhttp.open("GET", "php/add_new_medicine.php?name=" + name + "&packing=" + packing + "&suppliers_name=" + supplier_name, false);
  xhttp.send();
}

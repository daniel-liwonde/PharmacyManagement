...
alert(item_expires);
  if(!notNull(medicine_name.value, "medicine_name_error"))
    ...
    xhttp.open("GET", "php/manage_medicine.php?action=update&id=" + id + "&name=" + medicine_name.value + "&item_category=" + item_category.value + "&expires=" + item_expires.value + "&item_price=" + item_price.value, true);
    xhttp.send();
  }
}
